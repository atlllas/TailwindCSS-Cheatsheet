...
export default function GroupSection({ group }: { group: CheatGroup }) {
  ...
  return (
    <section
      id={group.slug}
      className="scroll-mt-6 rounded-lg border border-neutral-200 bg-white p-5 sm:p-6 compact:p-3 print:p-3 print:rounded-none print:border-0 print:border-t print:border-neutral-300 dark:border-neutral-800 dark:bg-neutral-900"
    >
      <h2 className="section-heading flex items-center gap-2 text-base font-semibold text-neutral-900 dark:text-neutral-100 mb-4 compact:mb-2 print:mb-1.5 print:text-sm print:pt-2">
        <CategoryIcon slug={group.slug} className="h-4 w-4 shrink-0 text-sky-600 dark:text-sky-400 print:hidden" />
        {group.title}
      </h2>
      <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 print:grid-cols-2 gap-x-8 print:gap-x-6 gap-y-3 compact:gap-y-1.5 print:gap-y-1">
        {entries.map((entry) => (
